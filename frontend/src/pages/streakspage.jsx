  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-800">Your Activity</h2>
          <p className="text-sm text-gray-600 mt-1">Last 365 days of task completion</p>
        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-lg p-4 sm:p-6 lg:p-8 border border-gray-200">
                <div className="mb-4 sm:mb-6">
                  ...
                  <p className="text-xs sm:text-sm text-gray-500">Your task completion over the last 90 days</p>
                </div>

                {/* Last 30 Days Grid */}
                {streakData.activityHistory && streakData.activityHistory.length > 0 ? (
                  <>
                  ...
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-sm mt-2">Complete tasks to see your progress here!</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white rounded-lg p-8 border border-gray-200">
                ...
                <div className="space-y-4">
                  {(() => {
                    // Get the most recent data (last 28 days from the end)
                    const allHistory = streakData.activityHistory || [];
                    ...
                    // Get last 28 days
                    const last28Days = allHistory.slice(-28);
                    
                    // Create 4 weeks (most recent week first)
                    const weeks = [];
                    for (let i = 3; i >= 0; i--) {
                      ...
                      if (weekDays.length > 0) {
                        ...
                        // Parse the last day of the week
                        const lastDay = weekDays[weekDays.length - 1];
                        const [endYear, endMonth, endDayNum] = lastDay.date.split('-').map(Number);
                        const weekEnd = new Date(endYear, endMonth - 1, endDayNum);
                    ...
                    return weeks.map((week, index) => (
                      <div key={index}>
                        <div className="flex items-center justify-between mb-2">
                          <span className={`text-sm font-medium ${week.isCurrentWeek ? 'text-indigo-600' : 'text-gray-700'}`}>
                            {week.label}
                          </span>
